...
CI.prototype.parseArgs = function (args) {
    ...
    if (args.length === 1 && args[0].indexOf(this.getConfig().CI_DOMAIN) === -1) {
...
CI.prototype.notifyChannel = function () {
    slackService.sendSimpleMessage(this.channelName, this.notifyMessage);
};
...
CI.prototype.triggerJob = function() {
    var param = querystring.stringify({ RELEASE_VERSION: this.version });
    var url = this.jobUrl;
    ...
    this.notifyMessage = 'Triggering `' + this.jobName + '` job for ' + this.productName;
    this.notifyChannel();
    http.get(url);
};
